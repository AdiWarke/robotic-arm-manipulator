...
void move_robot(const std::shared_ptr<rclcpp::Node> node){
    ...
    bool arm_plan_succsess = arm_move_group.plan(arm_plan) == moveit::core::MoveItErrorCode::SUCCESS;
    bool gripper_plan_success = gripper_move_group.plan(gripper_plan) == moveit::core::MoveItErrorCode::SUCCESS;

    if(arm_plan_succsess && gripper_plan_success){
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Planner Succed, moving the arm and the gripper");
        arm_move_group.move();
